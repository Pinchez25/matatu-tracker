...
/**
 * HTTP endpoints so you can manually trigger Kafka events without waiting
 * for the simulator. Great for learning and debugging.
 * <p>
 * Delegates to the producer services rather than KafkaTemplate directly —
 * the same separation of concerns you'd use in any layered Spring app.
 */
@RestController
...
public class MatatuController {
    ...
    public ResponseEntity<String> publishLocation(@RequestBody LocationEventRequest req) {
        var event = new LocationEvent(
                req.matatuId(), req.routeId(), req.routeName(),
                req.latitude(), req.longitude(), req.speedKmh(),
                req.passengersOnboard(), Instant.now()
        );
        locationProducer.send(event);
        return ResponseEntity.accepted().body("Location event queued for matatu: " + req.matatuId());
    }
     ...
     * -H "Content-Type: application/json" \
     * -d '{"matatuId":"KBZ 123A","amountKes":50,"paymentMethod":"MPESA","status":"SUCCESS"}'
     */
    @PostMapping("/fare")
    public ResponseEntity<String> publishFare(@RequestBody FareEventRequest req) {
        var event = new FareEvent(
