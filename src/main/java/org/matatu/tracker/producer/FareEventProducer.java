 ...
 * <p>
 * Note that both producers share the same pattern — KafkaTemplate, async send,
 * callback logging. This is intentional: learning to recognise this pattern
 * means you can extend it to any new event type.
 */
@Service
...
public class FareEventProducer {
    ...
    public CompletableFuture<SendResult<String, Object>> send(FareEvent event) {
        ...
        future.whenComplete((result, ex) -> {
            if (ex != null) {
                log.error("Failed to publish fare event [txn={}] for matatu [{}]: {}",
                        event.transactionId(), event.matatuId(), ex.getMessage());
            } else {
                log.debug("Published fare event → topic={}, partition={}, offset={}, txn={}, status={}",
