 ...
 * <p>
 * In a real system each physical matatu device would be its own Kafka producer.
 * Here we simulate the entire fleet from a single scheduled method — the Kafka
 * concepts are identical.
 * <p>
 * Notice this class delegates to {@link LocationEventProducer} and {@link FareEventProducer}
 * rather than touching KafkaTemplate directly. This is the right layering:
 * Simulator (what to send) → Producer service (how to send) → KafkaTemplate (send it)
 * <p>
 * KEY CONCEPT — Partitioning in action:
 * Watch the logs and notice that events for the same routeId always land on
 * the same partition number. That's Kafka's key-based partitioning at work.
 */
@Component
...
public class MatatuFleetSimulator {
    ...
    private static final List<RouteInfo> ROUTES = List.of(
            new RouteInfo("route_33", "Route 33", "CBD → Kikuyu", new double[]{-1.2921, 36.8219}),
            new RouteInfo("route_23", "Route 23", "CBD → Westlands", new double[]{-1.2650, 36.8100}),
            new RouteInfo("route_58", "Route 58", "CBD → Kawangware", new double[]{-1.2800, 36.7500}),
            new RouteInfo("route_111", "Route 111", "CBD → Rongai", new double[]{-1.4200, 36.7800}),
            new RouteInfo("route_46", "Route 46", "CBD → Eastleigh", new double[]{-1.2780, 36.8450})
    );

    // Simulated matatus — each belongs to a route
    private static final List<MatatuInfo> MATATUS = List.of(
    ...
    /**
     * Fires every 3 seconds, sending a GPS ping for each matatu in the fleet.
     * Adjust the rate in application.yml via app.simulator.gps-interval-ms.
     */
    ...
    /**
     * Fires every 5 seconds, simulating fare payments across the fleet.
     * Occasionally generates a FAILED payment to make the logs interesting.
     */
